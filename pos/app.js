...
let __A33_PC_EVENT_REFRESH_AT_MS = 0;

function pcMarkEventRefreshPending(nextId){
  __A33_PC_EVENT_REFRESH_PENDING = true;
  __A33_PC_EVENT_REFRESH_TARGET = (nextId == null || nextId === '') ? null : nextId;
  __A33_PC_EVENT_REFRESH_AT_MS = Date.now();
}

function pcClearEventRefreshPending(){
  __A33_PC_EVENT_REFRESH_PENDING = false;
  __A33_PC_EVENT_REFRESH_TARGET = null;
  __A33_PC_EVENT_REFRESH_AT_MS = 0;
}

function pcIsEventRefreshPending(){
  if (!__A33_PC_EVENT_REFRESH_PENDING) return false;
  const last = getPcLastRenderEventId();
  const tgt = __A33_PC_EVENT_REFRESH_TARGET;
  const aligned = (last == null && tgt == null) || (last != null && tgt != null && String(last) === String(tgt));
  if (aligned){
    pcClearEventRefreshPending();
    return false;
  }
  return true;
}

function pcRefreshPendingAgeMs(){
  return __A33_PC_EVENT_REFRESH_AT_MS ? (Date.now() - __A33_PC_EVENT_REFRESH_AT_MS) : 0;
}

function pcClearEventRefreshPendingIfAligned(renderedEventId){
  if (!__A33_PC_EVENT_REFRESH_PENDING) return;
  const tgt = __A33_PC_EVENT_REFRESH_TARGET;
  const rid = (renderedEventId == null || renderedEventId === '') ? null : renderedEventId;
  const aligned = (tgt == null && rid == null) || (tgt != null && rid != null && String(tgt) === String(rid));
  if (aligned) pcClearEventRefreshPending();
}

function pcToastUpdatingEvent(){
  try{ showToast('Actualizando evento…', 'info', 2200); }catch(_){ try{ toast('Actualizando evento…'); }catch(__){} }
}

function pcHasOptionValue(sel, value){
  try{
    const v = (value == null) ? '' : String(value);
    return Array.from(sel.options || []).some(o => String(o.value) === v);
  }catch(_){
    return false;
  }
}

function pcCaptureInputsForRetry(){
  const snap = {};
  try{
    const nodes = document.querySelectorAll(
      '#pc-table-nio input[type="number"], #pc-table-usd input[type="number"], ' +
      '#pc-table-fnio input[type="number"], #pc-table-fusd input[type="number"], ' +
      '#pc-mov-fieldset input, #pc-mov-fieldset select, #pc-event-fx-rate'
    );
    nodes.forEach(el => {
      if (!el || !el.id) return;
      // No capturamos el día (operativo/histórico) para no interferir con el cambio.
      if (el.id === 'pc-day' || el.id === 'pc-history-select') return;
      snap[el.id] = (el.type === 'checkbox') ? (!!el.checked) : (el.value != null ? String(el.value) : '');
    });
  }catch(_){ }
  return snap;
}

function pcRestoreInputsForRetry(snap){
  if (!snap) return;
  try{
    Object.keys(snap).forEach(id => {
      ...
      else el.value = (snap[id] == null) ? '' : String(snap[id]);
    });
  }catch(_){ }
...
function pcDraftPersistSave(d){
  try{
    if (!d) return;
    if (typeof sessionStorage === 'undefined') return;
    sessionStorage.setItem(A33_PC_DRAFT_STORAGE_KEY, JSON.stringify(d));
  }catch(_){ }
}

function pcDraftPersistLoad(){
  try{
    if (typeof sessionStorage === 'undefined') return null;
    const raw = sessionStorage.getItem(A33_PC_DRAFT_STORAGE_KEY);
    if (!raw) return null;
    const d = JSON.parse(raw);
    return (d && typeof d === 'object') ? d : null;
  }catch(_){ return null; }
...
function pcDraftPersistClear(){
  try{
    if (typeof sessionStorage === 'undefined') return;
    sessionStorage.removeItem(A33_PC_DRAFT_STORAGE_KEY);
  }catch(_){ }
}

function pcGetDraftEventIdCandidate(){
  // Preferir el selector si existe, para evitar guardar draft bajo un evento viejo en transiciones.
  try{
  ...
  try{
    const rid = getPcLastRenderEventId();
    if (rid) return rid;
  }catch(_){ }
  return null;
}

function pcInitialSnapMismatch(uiSnap, initSection){
  // Compara cantidades por denominacion (normalizado). Si hay mismatch devuelve detalle, si no, null.
  const snap = uiSnap || {};
  const init = initSection || {};

  try{
    if (typeof NIO_DENOMS !== 'undefined' && Array.isArray(NIO_DENOMS))
      for (const d of NIO_DENOMS){
        const id = 'pc-nio-q-' + d;
        const raw = Number(snap[id] || 0);
        const exp = (Number.isFinite(raw) && raw > 0) ? Math.floor(raw) : 0;
        const gotRaw = Number(init.nio && init.nio[String(d)] != null ? init.nio[String(d)] : 0);
        const got = (Number.isFinite(gotRaw) && gotRaw > 0) ? Math.floor(gotRaw) : 0;
        if (exp !== got) return { currency:'NIO', denom:d, expected:exp, got:got };
      }
  }catch(_){ }
  ...
  try{
    if (typeof USD_DENOMS !== 'undefined' && Array.isArray(USD_DENOMS))
      for (const d of USD_DENOMS){
        ...
        const raw = Number(snap[id] || 0);
        const exp = (Number.isFinite(raw) && raw > 0) ? Math.floor(raw) : 0;
        const gotRaw = Number(init.usd && init.usd[String(d)] != null ? init.usd[String(d)] : 0);
        const got = (Number.isFinite(gotRaw) && gotRaw > 0) ? Math.floor(gotRaw) : 0;
        if (exp !== got) return { currency:'USD', denom:d, expected:exp, got:got };
      }
  }catch(_){ }
...
function pcCaptureInitialDenomsSnap(){
  const snap = {};
  try{
    if (typeof NIO_DENOMS !== 'undefined' && Array.isArray(NIO_DENOMS))
      NIO_DENOMS.forEach(v=>{
        const el = document.getElementById('pc-nio-q-'+v);
        if (el) snap['pc-nio-q-'+v] = (el.value == null) ? '' : String(el.value);
      });
  }catch(_){ }
  try{
    if (typeof USD_DENOMS !== 'undefined' && Array.isArray(USD_DENOMS))
      USD_DENOMS.forEach(v=>{
        const el = document.getElementById('pc-usd-q-'+v);
        if (el) snap['pc-usd-q-'+v] = (el.value == null) ? '' : String(el.value);
      });
  }catch(_){ }
  return snap;
}

function pcRestoreInitialDenomsSnap(snap){
  if (!snap) return;
  try{
    Object.keys(snap).forEach(id => {
      const el = document.getElementById(id);
...
function pcSetDraftInitial(evId, dayKey, snap, reasonKey){
  const eid = parseInt(String(evId || ''), 10);
  const dk = String(dayKey || '').trim();
  if (!(Number.isFinite(eid) && eid > 0) || !dk) return;
  __A33_PC_DRAFT_INITIAL = { evId: eid, dayKey: dk, snap: snap || {}, reasonKey: reasonKey || null, atMs: Date.now() };
  try{ pcDraftPersistSave(__A33_PC_DRAFT_INITIAL); }catch(_){ }
}

function pcClearDraftInitial(evId, dayKey){
  // Si no pasan args, limpiar todo.
  if (evId == null && dayKey == null){ __A33_PC_DRAFT_INITIAL = null; try{ pcDraftPersistClear(); }catch(_){ } return; }
  const d = __A33_PC_DRAFT_INITIAL;
  if (!d) return;
  const eid = parseInt(String(evId || ''), 10);
  const dk = String(dayKey || '').trim();
  if ((Number.isFinite(eid) && String(d.evId) === String(eid)) && (!dk || String(d.dayKey) === String(dk))) { __A33_PC_DRAFT_INITIAL = null; try{ pcDraftPersistClear(); }catch(_){ } }
}

function pcRestoreDraftInitialIfMatch(evId, dayKey){
  let d = __A33_PC_DRAFT_INITIAL;
  if (!d){
    try{ d = pcDraftPersistLoad(); if (d) __A33_PC_DRAFT_INITIAL = d; }catch(_){ d = null; }
  }
  if (!d) return false;
  const now = Date.now();
  if (d.atMs && (now - d.atMs) > A33_PC_DRAFT_TTL_MS){ __A33_PC_DRAFT_INITIAL = null; try{ pcDraftPersistClear(); }catch(_){ } return false; }
  const eid = parseInt(String(evId || ''), 10);
  const dk = String(dayKey || '').trim();
  if (!(Number.isFinite(eid) && eid > 0) || !dk) return false;
  if (String(d.evId) !== String(eid) || String(d.dayKey) !== String(dk)) return false;
  pcRestoreInitialDenomsSnap(d.snap);
  return true;
}

function pcUpdateDraftInitialFromUI(reasonKey){
  try{
    const evId = pcGetDraftEventIdCandidate();
    if (!evId) return;
...
async function pcForceAlignCajaChicaToActiveEvent(activeId, opts){
  ...
  const snap = allowRestore ? pcCaptureInputsForRetry() : null;

  // Forzar selector (si existe) a reflejar el evento activo real.
  try{
    const sel = document.getElementById('pc-event-select');
    if (sel){
      if (activeId && pcHasOptionValue(sel, activeId)) sel.value = String(activeId);
      else sel.value = '';
    }
  }catch(_){ }

  // Re-render para alinear UI y caches internos.
  ...
  if (allowRestore){
    try{
      ...
      const ok = ((curId == null && activeId == null) || (curId != null && activeId != null && String(curId) === String(activeId)))
        && (!!selVal && String(selVal) === String(activeId));
      if (ok) pcRestoreInputsForRetry(snap);
    }catch(_){ }
  }
}
function setPcLastRenderEventId(id){
  if (id == null){ __A33_PC_LAST_RENDER_EVENT_ID = null; return; }
  const n = parseInt(String(id), 10);
  __A33_PC_LAST_RENDER_EVENT_ID = Number.isFinite(n) ? n : null;
}
function getPcLastRenderEventId(){
...
let __A33_PC_DIAG = {
  ...
  uiEventName: null,
  lastAction: null,
  lastResult: null, // ok | blocked | error | running
  lastReason: null,
  lastAtISO: null,
  lastAtDisplay: null,
  lastDetails: null,
...
function pcDiagResultLabel(r){
  const v = String(r || '').toLowerCase();
  if (v === 'ok') return 'OK';
  if (v === 'blocked') return 'BLOQUEADO';
  if (v === 'error') return 'ERROR';
  if (v === 'running') return 'EN PROCESO';
  return '—';
}

function pcDiagResultClass(r){
  const v = String(r || '').toLowerCase();
  if (v === 'ok') return 'pc-badge-ok';
  if (v === 'blocked') return 'pc-badge-warn';
  if (v === 'error') return 'pc-badge-err';
  if (v === 'running') return 'pc-badge-run';
  return '';
}
...
function pcHashPettySectionContent(section){
  try{
    const s = normalizePettySection(section || null) || {};
    const sortKeys = (obj)=> Object.keys(obj || {}).sort((a,b)=>{
      const na = Number(a), nb = Number(b);
    ...
    const pack = (obj)=> sortKeys(obj).map(k=>`${k}:${Number(obj[k]||0)}`).join(',');
    const str = `nio:${pack(s.nio)}|usd:${pack(s.usd)}`;
    const hash = a33HashStringPOS(str);
    return { hash: hash ? String(hash) : null };
  }catch(_){
    return { hash: null };
  }
}

function pcComputeMonotonicISO(prevISO){
  try{
    const now0 = Date.now();
    let prevMs = null;
    if (prevISO){
      const t = new Date(String(prevISO)).getTime();
      if (!Number.isNaN(t)) prevMs = t;
    }
    let ms = now0;
    if (prevMs != null && Number.isFinite(prevMs) && ms <= prevMs){
      ms = prevMs + 1;
    }
...
const A33_PC_IDB_TIMEOUT_MS = 6500;
function pcWithTimeout(promise, ms, code, msg){
  const t = Number(ms || A33_PC_IDB_TIMEOUT_MS);
  const c = code || A33_PC_TECH_CAUSES.IDB_TIMEOUT;
  return new Promise((resolve, reject)=>{
    ...
    const timer = setTimeout(()=>{
      ...
      try{ e.code = c; }catch(_){ }
      try{ e.name = 'TimeoutError'; }catch(_){ }
      reject(e);
    }, (Number.isFinite(t) && t > 0) ? t : A33_PC_IDB_TIMEOUT_MS);
...
function pcDiagRender(){
  ...
  const uiEl = document.getElementById('pc-diag-ui');
  ...
  if (resEl){
    resEl.textContent = pcDiagResultLabel(d.lastResult);
    resEl.classList.remove('pc-badge-ok','pc-badge-warn','pc-badge-err','pc-badge-run');
    const cls = pcDiagResultClass(d.lastResult);
...
function pcDiagUpdateContext(activeId, activeName, uiId, uiName){
  __A33_PC_DIAG.activeEventId = activeId || null;
  __A33_PC_DIAG.activeEventName = activeName || null;
  __A33_PC_DIAG.uiEventId = uiId || null;
  __A33_PC_DIAG.uiEventName = uiName || null;
  try{ pcDiagRender(); }catch(e){}
  try{ pcDiagRenderLog(); }catch(_){ }
}

function pcDiagMark(action, result, reason, details){
  const now = new Date();
  let stamp = null;
  try{ stamp = fmtDDMMYYYYHHMM_POS(now); }catch(e){ stamp = now.toLocaleString(); }

  __A33_PC_DIAG.lastAction = action || null;
  __A33_PC_DIAG.lastResult = result || null;
  __A33_PC_DIAG.lastReason = reason || null;
  __A33_PC_DIAG.lastAtISO = now.toISOString();
  ...
  try{
    pcDiagLogAppend({
      ...
      atDisplay: __A33_PC_DIAG.lastAtDisplay,
      action: __A33_PC_DIAG.lastAction,
      result: __A33_PC_DIAG.lastResult,
      reason: __A33_PC_DIAG.lastReason,
      activeEventId: __A33_PC_DIAG.activeEventId,
      uiEventId: __A33_PC_DIAG.uiEventId,
      details: __A33_PC_DIAG.lastDetails
    });
  }catch(_){ }
...
function pcDiagLogAppend(entry){
  ...
  if (!e) return;
  const arr = pcDiagLogLoad();
  arr.push(e);
  while (arr.length > 20) arr.shift();
  pcDiagLogSave(arr);
}

function pcDiagTrunc(s, maxLen){
  const t = String(s || '');
...
function pcDiagRenderLog(){
  const box = document.getElementById('pc-diag-log');
  if (!box) return;
  const arr = pcDiagLogLoad();
  ...
  for (const it of last){
    ...
    const at = it.atDisplay || '';
    const act = it.action || '';
    const res = pcDiagResultLabel(it.result);
    const why = it.reason || '';

    left.textContent = `${at} · ${act}`;
    right.textContent = `${res} · ${pcDiagTrunc(why, 90)}`;
...
async function renderPcEventSwitchUI(dayKey){
  ...
  if (currentId && items.some(i => String(i.id) === String(currentId))){
    ...
    // Fuente única de verdad: si el evento activo no está disponible, no dejar el selector apuntando a otro.
    sel.value = '';
  }
...
async function ensurePettyEnabledForEvent(eventId){
  const ev = await getEventByIdSafe(eventId);
  if (!ev){
    showToast('Evento no encontrado.', 'error', 4500);
    return false;
  }
  if (ev.closedAt){
...
async function guardPettyWritePOS(eventId, opts){
  ...
  const toastErr = (t)=>{
    if (silent) return;
    try{ showToast(t, 'error', 4500); }catch(e){
      try{ toast(t); }catch(_){
        try{ alert(t); }catch(__){}
      }
    }
  };
...
async function isPettyDayLockedPOS(evId, dayKey, pc){
  let lock = null;
  try{ lock = await getDayLockRecordPOS(evId, dayKey); }catch(_){ lock = null; }
  const day = ensurePcDay(pc, dayKey);
  const legacyClosed = !!(day && day.closedAt);
  const unifiedClosed = !!(lock && lock.isClosed);
  return unifiedClosed || legacyClosed;
}
...
function pcFinStatusClearStickyIfContextMismatch(evId, dayKey){
  ...
  if (!s) return;
  if (String(s.dataset.sticky || '') !== '1') return;
  const se = String(s.dataset.stickyEvId || '');
  const sd = String(s.dataset.stickyDayKey || '');
  const ce = String(evId || '');
  const cd = String(dayKey || '');
  if (se === ce && sd === cd) return;
  s.dataset.sticky = '';
  s.dataset.stickyEvId = '';
  s.dataset.stickyDayKey = '';
  s.textContent = '';
}

function pcFinStatusIsStickyForContext(evId, dayKey){
  const s = document.getElementById('pc-init-from-fin-status');
  if (!s) return false;
  if (String(s.dataset.sticky || '') !== '1') return false;
  const se = String(s.dataset.stickyEvId || '');
  const sd = String(s.dataset.stickyDayKey || '');
  const ce = String(evId || '');
  const cd = String(dayKey || '');
...
function pcFinStatusSet(msg, { html=false, sticky=false, evId=null, dayKey=null } = {}){
  const s = document.getElementById('pc-init-from-fin-status');
  if (!s) return;

  if (sticky){
    s.dataset.sticky = '1';
    s.dataset.stickyEvId = (evId == null) ? '' : String(evId);
    s.dataset.stickyDayKey = (dayKey == null) ? '' : String(dayKey);
  } else {
    s.dataset.sticky = '';
    s.dataset.stickyEvId = '';
    s.dataset.stickyDayKey = '';
...
function updatePcFinSyncUI(ev, canInteract, readOnlyDay, dayKey, blockState){
  ...
  if (!st.ok){
    if (st.code === 'FIN_NOT_CONFIG' || st.code === 'FIN_NOT_READY'){
      ...
      pcFinStatusSet(st.msg || 'Finanzas no configurada.');
    }
    return;
  }
...
async function renderCajaChica(){
  ...
  if (!evId || !ev){
    setPcLastRenderEventId(null);
    try{ pcDiagUpdateContext(null, null, null, null); pcDiagRender(); }catch(_){ }
  ...
  const eventClosed = !!ev.closedAt;

  const dayKey = getSelectedPcDay();
  const hist = isPettyHistoryMode();
  lbl.textContent = 'Evento activo: ' + ev.name + (eventClosed ? ' · Evento cerrado' : '') + ' · ' + (hist ? ('Histórico: ' + dayKey) : ('Día: ' + dayKey));
  note.style.display = 'none';
  if (eventClosed) main.classList.add('disabled');
...
async function onSavePettyInitial(opts){
  ...
  // Detectar SIN CAMBIOS por contenido (ignorar savedAt)
  const beforeContent = pcHashPettySectionContent(rawBeforeDay ? rawBeforeDay.initial : (dayBefore || {}).initial).hash;
  const newContent = pcHashPettySectionContent({ nio, usd, savedAt: null }).hash;

  if (beforeContent && newContent && beforeContent === newContent){
    // Caso M2: mismos valores => no es error
    try{ pcClearDraftInitial(evId, canonicalDayKey); }catch(_){ }
    const pcSame = await getPettyCash(evId);
    const cashSalesNioSame = await getCashSalesNioForDay(evId, dayKey);
    updatePettySummaryUI(pcSame, dayKey, { cashSalesNio: cashSalesNioSame });
    fillPettyInitialFromPc(pcSame, dayKey);
  ...
  if (!okByHash){
    // WRITE_NO_CHANGE solo si realmente no cambió lo persistido
  ...
  return true;
    ...
    const tech = (code === A33_PC_TECH_CAUSES.IDB_BLOCKED)
      ...
      : (code === A33_PC_TECH_CAUSES.IDB_TIMEOUT)
        ? 'El acceso a IndexedDB excedió el tiempo. Cerrá otras pestañas y reintentá.'
        : (m.detail || 'Error de persistencia.');

    try{
      showToast((code === A33_PC_TECH_CAUSES.IDB_BLOCKED)
        ? 'IndexedDB bloqueada (otra pestaña). Cerrá otras pestañas y reintentá.'
        : (code === A33_PC_TECH_CAUSES.IDB_TIMEOUT)
          ? 'Timeout de IndexedDB. Reintentá.'
          : 'Error de persistencia.', 'error', 6500);
    }catch(_){ }

    diag('error', 'No se pudo guardar (IDB).', {
      causeCode: code,
      techDetail: tech,
      forensic: mkFor({ eventId: evId, extra: 'err: ' + String(err || '') })
    });
...
function bindCajaChicaEvents(){
  ...
  inputsInit.forEach(inp=>{
    const onDraft = (k)=>{
      try{ recalcPettyInitialTotalsFromInputs(); }catch(_){ }
      try{ pcUpdateDraftInitialFromUI(k); }catch(_){ }
    };
  ...
  if (finTog){
    finTog.addEventListener('change', async ()=>{
      ...
      if (ev.closedAt){
        ...
        toast('Este evento está cerrado.');
        return;
      }
      if (!eventPettyEnabled(ev)){
        finTog.checked = !!ev.pcInitFromFinanzasEnabled;
